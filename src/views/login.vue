  ...
  <div class="login-wrap">
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
      ...
      <el-form-item prop="pwd">
        <el-input type="pwd" v-model="ruleForm.pwd" auto-complete="off" placeholder="密码" @keydown.native.enter="submitForm('ruleForm')"></el-input>
      </el-form-item>
    ...
    methods: {
      ...
      getuserpwd() {
        if (getCookie('account') != '' && getCookie('pwd') != '') {
          this.ruleForm.account = getCookie('account')
          ...
          if (valid) {
            login(this.ruleForm).then(async res => {
              if (res.code === 0) {
                if (this.rememberpwd) {
                  //保存帐号到cookie，有效期7天
                  await setCookie('account', this.ruleForm.account, 7)
                  //保存密码到cookie，有效期7天
                  await setCookie('pwd', this.ruleForm.pwd, 7)
                } else {
                  await delCookie('account')
                  await delCookie('pwd')
                }
                await sessionStorage.setItem('userAccount', res.data.userAccount)
                await sessionStorage.setItem('logintoken', res.data.token)
                await sessionStorage.setItem('userId', res.data.userId)
                await sessionStorage.setItem('nickName', res.data.userNickname)
                await sessionStorage.setItem('shopName', res.data.shopName)
                await sessionStorage.setItem('userType', res.data.userType)
                if (res.data.shopId) {
                  await sessionStorage.setItem('shopId', res.data.shopId)
                } else {
                  await sessionStorage.removeItem('shopId')
                }
                /*if (res.data.studioList) {
                  await sessionStorage.setItem('studioList', JSON.stringify(res.data.studioList))
                }else{
                  await sessionStorage.removeItem('studioList')
                }*/
                // menuList 接口
                // await this.$router.push({path: '/index'})
                if(res.data.userType == 'system') {
                  await this.$router.push({path: '/order/order'})
                } else{
                  await this.$router.push({path: '/tmOrderForm/index'})
                }
              } else {
